...
export class ProfileExtractProcessor extends WorkerHost {
  ...
  async process(job: Job<ProfileExtractPayload>): Promise<unknown> {
    ...
    this.logger.log(
      `extracting profile for author ${authorId} platform=${platform ?? 'auto'} (job ${job.id})`,
    );
    ...
    try {
      ...
      const entries = Object.entries(socials) as [
        string,
        { value: string; synchronize: boolean } | undefined,
      ][];

      const chosen =
        this.pickPlatform(entries, platform) ??
        (entries.find(([, v]) => v?.synchronize && v.value) ??
          entries.find(([, v]) => v?.value))?.[0];

      if (!chosen) {
        throw new NotFoundException(
          `Author ${authorId} has no social handle to extract`,
        );
      }
      if (!SUPPORTED.includes(chosen as SupportedPlatform)) {
        throw new Error(
          `Unsupported platform '${chosen}' for profile extraction`,
        );
      }
      ...
      if (!handle) {
        throw new NotFoundException(
          `Author ${authorId} has no handle for platform ${chosen}`,
        );
      }

      const raw = await this.fetchProfile(chosen as SupportedPlatform, handle);
      this.logger.log(
        `profile fetched for ${author.name} @ ${chosen}/${handle} (job ${job.id})`,
      );

      const { profileData, links } = this.mapProfile(
      ...
      const [upserted] = await this.db
        ...
        .values({
          authorId,
          platform: chosen as SupportedPlatform,
          ...profileData,
        })
        .onConflictDoUpdate({
      ...
      if (links.length > 0) {
        await this.db
          .delete(authorsProfileLinks)
          .where(eq(authorsProfileLinks.profileId, upserted.id));
      ...
      this.logger.log(
        `profile persisted for author ${authorId} @ ${chosen}/${handle} (job ${job.id})`,
      );
      return { authorId, platform: chosen, handle, profileData };
    } catch (err) {
      this.logger.error(
        `profile-extract job ${job.id} failed for author ${authorId} platform=${platform ?? 'auto'}: ${(err as Error).message}`,
        (err as Error).stack,
      );
      throw err;
    }
  ...
  private mapProfile(
    ...
    profileData: Omit<NewAuthorProfile, 'authorId' | 'platform'>;
    links: { url: string; title?: string }[];
  } {
    const base: Omit<NewAuthorProfile, 'authorId' | 'platform'> = {
      handle,
      raw,
    };
      ...
      case 'tiktok': {
        ...
        return {
          profileData: {
            ...
            externalUrl:
              (u['bioLink'] as { link?: string } | undefined)?.link ?? null,
